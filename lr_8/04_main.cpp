...
extern "C" void run_naive(const float *A, const float *B, float *C, int n);
extern "C" void run_row_cache(const float *A, const float *B, float *C, int n);
extern "C" void run_col_cache(const float *A, const float *B, float *C, int n);
extern "C" void run_tiled(const float *A, const float *B, float *C, int n, int S);
...
int main(int argc, char **argv) {
    ...
    // Запуск различных алгоритмов
    test_gpu("Naive (16x16)", run_naive);
    if (N <= 1024) { // ограничение max threads per block = 1024
